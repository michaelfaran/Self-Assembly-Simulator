...
ENTR_vec=zeros(1,length(delays));
RR_vec=zeros(1,length(delays));
final_final_eps=zeros(1,length(delays));
drive=1;
Js=-4.5;

for kk=10:1:length(delays)
...
% end
% figure;
% plot(epslion_vec,(RR_vec,1));
% hold on;
% plot(epslion_vec,sum(ENTR_vec,1));

delay=floor(delays(kk));
d=dime_vec(kk);
...
[R,D]=rp(y,mega_eps_vec(ii),'fix','max');
R=R-diag(diag(R));
...
[pks,locs]=findpeaks(-ENTR_vec);
[M,I]=max(ENTR_vec(locs));
I2=find(ENTR_vec==M);
yy=embed(Total_energy2,dime_vec(I2),floor(delays(I2)));
[R,D]=rp(yy,final_final_eps(I2),'fix','max');
R=R-diag(diag(R));
index_y=embed(1:1:length(Total_energy2),dime_vec(I2),floor(delays(I2)));
