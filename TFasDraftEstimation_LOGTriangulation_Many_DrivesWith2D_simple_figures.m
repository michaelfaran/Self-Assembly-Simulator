...
drive_vec=1;
Mfit_vec=8.3*(10^3).*exp(-2.9*drive_vec)+4.6*10^2;
std_fit=3.1*(10^3).*exp(-2.2*drive_vec)+2.5*10^2;
coordinate_number=2;
...
%             subplot(4,1,1)
%             scatter(x,y)
%             ylabel('T_{FAS} First Subdivision')
%             Y=[Y y];
            % X = [ones(length(x),1) x];
            % X=x;
%             hold on;
...
%                     xlabel('T_{FAS} Predicted')
           ylabel(sprintf('$ {T}_{FAS} $'),'Interpreter','latex','FontSize',28);
%          hold on;
%                 b = x\(y);
%             yCalc2 = x*b;
%             plot(x,yCalc2,'--')
...
% 
% %         xlabel('T_{FAS} Predicted')
%         ylabel('T_{FAS} Cross Validation')
...
predictor=nan(1,length(hista)-1);
        for ii=1:1:(cutofflength-1)
            I=find (hista(ii)<x & x<(hista(ii+1)));
            if length(I)<3 %do nto consider when there is not enough statistics. here changed for small scale sake
            std_hista(ii)=nan;
            mean_hista(ii)=nan;
            else
               yo =sort(y_new(I));
...
set(gca,'xticklabel',[])
 set(gca,'FontSize',20);
          yy= ylabel(sprintf('$ {T}_{FAS} $'),'Interpreter','latex','FontSize',28);
          set(yy, 'Units', 'Normalized', 'Position', [-0.1, 0.5, 0]);

yticks([0:1000:5000])
subplot(2,1,2)  
        h1=plot(x_hista(1:end),mean_hista(1:end),'r','LineWidth',4);
        hold on;
        h2=plot(x_hista(1:end),std_hista(1:end),'b','LineWidth',4);
                    ylim([0 2000])
        hold on;
        h3=plot(x_hista(1:cutofflength),x_hista(1:cutofflength),'k','LineWidth',3);
...
yticks([0:500:2000]);
  ...
  yy=ylabel(sprintf('${T}_{FAS} $'),'Interpreter','latex','FontSize',28);
          ...
          xx= xlabel(sprintf('$ \\hat{T}_{FAS} $'),'Interpreter','latex','FontSize',28);
%             set(xx, 'Units', 'Normalized', 'Position', [0.5, -0.3, 0]);

  legend([h1 h2 h3 ],'Predictor Result','Predictor Error','Linear Compare','FontSize',16)
fullscreen()
% han=axes(figg,'visible','off'); 
% han.Title.Visible='on';
% han.XLabel.Visible='on';
% han.YLabel.Visible='on';
% ylabel(han,'T_{FAS} Measured');
%  xlabel(han,'T_{FAS} Predicted');
%  set(gca,'FontSize',28);
folderName2=horzcat(Main_dir_hazirim,'\',folderName);
 sgtitle(horzcat('\Delta \mu = ',num2str(mu, '%5.1f'),', M_{fit} = ',num2str(Mfit_vec(kok), '%5.1f'),', \sigma_{fit} = ',num2str(std_fit(kok), '%5.1f')),'FontSize',20);
