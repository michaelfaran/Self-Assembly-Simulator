...
%                     xlabel('T_{FAS} Predicted')
        ylabel('T_{FAS} Measured All Subdivisions')
%          hold on;
%                 b = x\(y);
%             yCalc2 = x*b;
%             plot(x,yCalc2,'--')
...
subplot(4,1,4)  
        h1=plot(x_hista(1:end),mean_hista(1:end),'r','MarkerSize',20);
        hold on;
        h2=plot(x_hista(1:end),std_hista(1:end),'b','MarkerSize',20);
                    ylim([0 2000])
        hold on;
        h3=plot(x_hista(1:cutofflength),x_hista(1:cutofflength),'k');
