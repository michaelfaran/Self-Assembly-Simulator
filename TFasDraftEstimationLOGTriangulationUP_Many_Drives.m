...
%         std_hista(ii)=std(y_new(I));
       ...
       x_hista=(hista(2:end)+hista(1:(end-1)))/2;
        I2=find (isnan(mean_hista) & x_hista>100,1);
        I3=find (isnan(std_hista) &  x_hista>100,1);
        I4=min(I2,I3);
    subplot(3,1,3)  
...
savefig(figure(gcf),fullfile(Main_dir_hazirim,horzcat('Predictor_', regexprep(num2str(mu, '%5.1f'),'\.','_'),'.fig')),'compact');
saveas(gcf,fullfile(Main_dir_hazirim,horzcat('Predictor_', regexprep(num2str(mu, '%5.1f'),'\.','_'),'.png')),'png');
end
