...
C = strsplit(aa,'\n');
pivot_i=1;
%beginning run with mu =0.4 and with interaction =-4.0------
gg=horzcat('beginning run with mu =',num2str(mu),' and with interaction =',num2str(J,formatSpec),'------');
for mm=1:1:length(C)
tf = contains(C{1,mm},'tfas');

%michael add towards june
if tf==1
    for jij=1:1:num_of_runs_per_one
        try
        if contains(C{1,mm-jij*(num_of_runs_per_one-1)-1},gg)
          tf=1;
          break;
        else
            tf=0;
