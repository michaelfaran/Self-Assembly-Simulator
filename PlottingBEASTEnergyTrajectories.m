...
%Previously,calculated the first minimum of the mutual infromation to
%describe delay and was named "Mutual_entropy_Calc", now no need for this
%anymore in the new method.
%I converted this into "the up
%Oudated plotter of energy trajectories with
%BEAST activated on them".
...
%the vector for the sake of the BEAST to operate well. Here is different
%than the newer codes, but this is just for plotting the enrgy trajectories
%so it does not matter much. 

%make sure before the plot that the changes here are similar to the changes
%put into the BEAST in other codes before plotting. 

mainname='06_11_2022_18_31_37_mu_1_Js__4_0_runs_3';
restoredefaultpath
addpath 'C:\Users\admin\Documents\Run_Matlab_Fast_Folder'
Main_dir_hazirim= 'C:\Users\admin\Documents\Run_Matlab_Fast_Folder\Mutual_Entropy_Calc';
cd(Main_dir_hazirim)
...
%6/10/22 here Michael canceled the others, if you want to do something
%rather than plot, please refer to gATHER_tfas_DATA_SINGLE_DRIVE etc
% cd(xxy);
% A_reduced=[];
% C_reduced=[];
% for ii=1:1:ssz(1)
